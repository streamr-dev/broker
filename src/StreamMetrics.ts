import { Stream, StreamMessageAsObject, StreamOperation, StreamrClient } from "streamr-client"
...
export class StreamMetrics {
    ...
    metricsReportTimeout?:NodeJS.Timeout
    ...
    async createMetricsStream(path:string) {
    ...
    async getResend(stream:string, last=1, timeout:number = 10 * 1000):Promise<Array<any>> {
        return new Promise((resolve, reject) => {
            ...
            const messages:Array<StreamMessageAsObject> = []
        ...
        try {
            ...
            if (this.interval === 'sec') {
                ...
                if (!this.sourceStreamId){
                    throw new Error(`Cannot report ${this.interval} withour [sourceStreamId]`)
                }
